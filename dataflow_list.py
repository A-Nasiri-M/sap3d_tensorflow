...
class VideoDataset():
    def __init__(self, frame_basedir, density_basedir, fixation_dir=None, img_size=(480,288), video_length=16, stack=5, bgr_mean_list=[103.939, 116.779, 123.68],sort='bgr'):
        ...
        # assert isinstance(self.frame_basedir, (list))
        self.density_basedir = density_basedir
        # self.video_dir_list = []
        # for each in self.frame_basedir: 
        self.video_dir_list = []
        for each_video in self.frame_basedir:
            self.video_dir_list+=glob.glob(os.path.join(each_video, "*"))
...
def mapf_test(dp):
    frame_list, density_list, fixation_list = dp
    ret_frame = []
    ret_density = []
    ret_fixation = []
    for frame in frame_list:
    ...
    for density in density_list:
        im = cv2.imread(density, cv2.IMREAD_GRAYSCALE)
        im = cv2.resize(im, (960, 1080))
        im = im / 255.
        ret_density.append(im)
    for fixation in fixation_list:
        im = cv2.imread(fixation, cv2.IMREAD_GRAYSCALE)
        im = im / 255.
        ret_fixation.append(im)
