...
if net == 'P3D':
    pred=p3d.inference_p3d(x, keep_var, batch_size, training)
elif net == 'P3D_SA': 
    pred=p3d.inference_p3d_sa(x, keep_var, batch_size, training)
...
with tf.Session(config=config) as sess:
    ...
    for valid_data in gt_df:
        ...
        for (prediction, ground_truth) in zip(image, valid_batch_ys):
            ...
            # ground_truth = np.array(zero_gt.append(ground_truth[-1]))
            # print np.array(prediction).shape, np.sum(prediction), np.array(zero_gt).shape, np.sum(zero_gt)
