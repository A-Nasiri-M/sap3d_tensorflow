...
def l2_reg(wd=0.0005, scope=None):
    return tf.contrib.layers.l2_regularizer(wd, scope=scope)

def conv3d_layers(x, filters, kernel, strides, name):
    x = tf.layers.conv3d(x, filters, kernel, strides, 'same', kernel_initializer=tf.contrib.layers.xavier_initializer(), kernel_regularizer=l2_reg(), name=name)
    x = GNReLU(x)
    return x

def deconv3d_layers(x, filters, kernel, strides, name):
    x = tf.layers.conv3d_transpose(x, filters, kernel, strides, 'same', kernel_initializer=tf.contrib.layers.xavier_initializer(), kernel_regularizer=l2_reg(),name=name)
    x = GNReLU(x)
    return x
...
class Bottleneck():
    ...
    def infer(self):
        ...
        if len(self.downsample)==1:
            ...
            residual=GroupNorm(residual)
        # residual + self-attention / CBAM ? 
        # residual = attention(residual, ch=None, name='attention_{}'.format(self.id))
        # CBAM
        residual = cbam_block(residual, name='cbam_{}'.format(self.id))
        out+=residual
        out=tf.nn.relu(out)
...
def inference_p3d_sa_concat_2(_X,_dropout,batch_size=2, training=True):
    ...
    deconv_pool3_gn = GNReLU(deconv_pool3, 'deconv_pool3_gn')
    ## attention 
    deconv_pool3_gn_sa = attention(deconv_pool3_gn, 256, 'deconv_pool3_gn_sa', False)
    ...
    conv_concat = tf.layers.dropout(conv_concat, rate=_dropout, training=training)
    conv_concat = GNReLU(conv_concat)
...
def inference_p3d_sa_decoder_block(_X,_dropout,batch_size=2, training=True):
    with tf.variable_scope('P3D'):
        ...
        pool3=tf.nn.max_pool3d(res2,[1,2,1,1,1],strides=[1,2,1,1,1],padding='SAME')
        # 2 14 14 512
        deconv_pool3 = deconv3d_layers(pool3, 512, [2, 3, 3], [2, 2, 2], 'deconv_pool3')
        deconv_pool3_sa = attention(deconv_pool3, 512, 'deconv_pool3_sa', False)
        b3=make_block(pool3,256,36,512,cnt,stride=2)
        res3=b3.infer()
        # 2 7 7 1024
        cnt=b3.cnt
        pool4=tf.nn.max_pool3d(res3,[1,2,1,1,1],strides=[1,2,1,1,1],padding='SAME')
        # from 1 7 7 to 4 28 28 1024
        deconv_pool4 = deconv3d_layers(pool4, 1024, [1, 3, 3], [4, 4, 4], 'deconv_pool4')
        deconv_pool4_sa = attention(deconv_pool4, 1024, 'deconv_pool4_sa', False)
        ###
        ### Concat
        ###
        concatenator = tf.concat([pool2_sa, deconv_pool3_sa, deconv_pool4_sa], axis=-1, name='concatenator')
        conv_concat = conv3d_layers(concatenator, 1024, 3, 1, 'conv_concat')

        # Decoder Block 1
        decoder1_conv1 = conv3d_layers(conv_concat, 256, 3, 1, 'decoder1_conv1')
        decoder1_deconv = deconv3d_layers(decoder1_conv1, 256, 3, 2, 'decoder1_deconv')
        decoder1_conv2 = conv3d_layers(decoder1_deconv, 128, 3, 1, 'decoder1_conv2')
        decoder1_conv2 = tf.layers.dropout(decoder1_conv2, _dropout, training=training)

        # Decoder Block 2
        decoder2_conv1 = conv3d_layers(decoder1_conv2, 32, 3, 1, 'decoder2_conv1')
        decoder2_deconv = deconv3d_layers(decoder2_conv1, 32, 3, 2, 'decoder2_deconv')
        decoder2_conv2 = conv3d_layers(decoder2_deconv, 16, 3, 1, 'decoder2_conv2')
        # Final Conv
        
        final_conv = tf.layers.dropout(decoder2_conv2, _dropout, training=training)
        results = tf.layers.conv3d(final_conv, 1, 3, 1, 'same', kernel_regularizer=l2_reg(), name='results')

        return results

def inference_p3d_decoder_block(_X,_dropout,batch_size=2, training=True):
    with tf.variable_scope('P3D'):
        cnt=0
        # 16 112 112 3
        conv1_custom=tf.nn.conv3d(_X,get_conv_weight('firstconv1',[1,7,7,3,64]),strides=[1,1,2,2,1],padding='SAME')
        ...
        cnt=b1.cnt
        pool2=tf.nn.max_pool3d(res1,[1,2,1,1,1],strides=[1,2,1,1,1],padding='SAME')
        deconv_pool2 = deconv3d_layers(pool2, 128, [3, 3, 3], [1, 1, 1], 'deconv_pool2')
        # 4 28 28 256
        b2=make_block(pool2,128,8,256,cnt,stride=2)
        res2=b2.infer()
        # 4 14 14 512 
        ...
        pool3=tf.nn.max_pool3d(res2,[1,2,1,1,1],strides=[1,2,1,1,1],padding='SAME')
        # 2 14 14 512
        deconv_pool3 = deconv3d_layers(pool3, 256, [2, 3, 3], [2, 2, 2], 'deconv_pool3')
        b3=make_block(pool3,256,36,512,cnt,stride=2)
        res3=b3.infer()
        # 2 7 7 1024
        cnt=b3.cnt
        pool4=tf.nn.max_pool3d(res3,[1,2,1,1,1],strides=[1,2,1,1,1],padding='SAME')
        # from 1 7 7 to 4 28 28 1024
        deconv_pool4 = deconv3d_layers(pool4, 512, [1, 3, 3], [4, 4, 4], 'deconv_pool4')
        ###
        ### Concat
        ###
        ...
        final_conv = tf.layers.dropout(decoder2_conv2, _dropout, training=training)
        results = tf.layers.conv3d(final_conv, 1, 3, 1, 'same', kernel_initializer=tf.contrib.layers.xavier_initializer(), kernel_regularizer=l2_reg(),name='results')
        return results
